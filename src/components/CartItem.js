...
const mapDispatchToProps = (dispatch, myOwnProps) => {
  const { id, amount } = myOwnProps;
  return {
    remove: () => dispatch({ type: REMOVE, payload: { key: id } }),
    increase: () => dispatch({ type: INCREASE, payload: { key: id } }),
    decrease: () => dispatch({ type: DECREASE, payload: { key: id, num_item: amount } })
  };
};
